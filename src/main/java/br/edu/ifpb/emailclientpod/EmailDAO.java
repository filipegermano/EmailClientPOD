...
public class EmailDAO {
    ...
    public void salvar(Email email) {
        try {
            String sql = "insert into email_cliente (remetente, destinatarios, assunto, mensagem, ipservidor) "
                    + "values (?, ?, ?, ?, ?)";
            ...
            pst.setString(5, email.getIpServidor());
            pst.execute();
            pst.close();
    ...
    public List<Email> emailsNaoEnviados() {
        try {
            ...
            String sql = "select id, remetente, destinatarios, assunto, mensagem, ipservidor "
                    + "from email_cliente where status = false";
            connection = conexao.createConnection();
            PreparedStatement pst = connection.prepareStatement(sql);
    ...
    public void setStatus(Email email) {
        try {
            String sql = "update email_cliente set status = true where id = ? ";
